...
namespace Ambition
{
    public class GuestRemarkResultView : GuestViewMediator
    {
        ...
        private PartyModel _partyModel = AmbitionApp.GetModel<PartyModel>();
        private ConversationModel _conversationModel = AmbitionApp.GetModel<ConversationModel>();
        ...
        void Awake()
        {
            ...
            AmbitionApp.Subscribe<GuestVO>(PartyMessages.GUEST_REACTION_ALREADYCHARMED, HandleAlreadyCharmed);
            AmbitionApp.Subscribe<GuestVO>(PartyMessages.GUEST_CHARMED, HandleCharmed);
            AmbitionApp.Subscribe<GuestVO>(PartyMessages.GUEST_REACTION_BORED, HandleBored);
            AmbitionApp.Subscribe<GuestVO>(PartyMessages.GUEST_LEFT, HandleLeft);
            InitGuest();
        }
        ...
        private void OnDestroy()
        {
            ...
            AmbitionApp.Unsubscribe<GuestVO>(PartyMessages.GUEST_REACTION_ALREADYCHARMED, HandleAlreadyCharmed);
            AmbitionApp.Unsubscribe<GuestVO>(PartyMessages.GUEST_CHARMED, HandleCharmed);
            AmbitionApp.Unsubscribe<GuestVO>(PartyMessages.GUEST_REACTION_BORED, HandleBored);
            AmbitionApp.Unsubscribe<GuestVO>(PartyMessages.GUEST_LEFT, HandleLeft);
        }
        ...
        private void HandleSelected(GuestVO guest)
        {
            ...
            if (key == PartyConstants.DISLIKE && _conversationModel.ItemEffect)
            {
        ...
        private void HandleAlreadyCharmed(GuestVO guest)
        {
        ...
        private void HandleCharmed(GuestVO guest)
        {
            if (guest != null && Guest == guest)
            {
        ...
        private void HandleBored(GuestVO guest)
        {
            if (guest != null && Guest == guest)
            {
        ...
        private void HandleLeft(GuestVO guest)
        {
            if (guest != null && Guest == guest)
            {
        ...
        private string _key(RemarkVO remark, GuestVO guest)
        {
        ...
        {
            string str = localizationModel.GetString(dialogString, dictionary);
            if (str != null && Text != null) return str;
            else return localizationModel.GetString(dialogString);
        }
    }
}
